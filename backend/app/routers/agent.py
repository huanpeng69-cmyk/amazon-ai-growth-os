...
P1-4：所有重端点改为 ``async def``，外部 IO（Agnes / Bright Data）经
``run_blocking_with_timeout`` 在线程中执行并整体超时——事件循环在长调用期间
不被阻塞，超阈值即返回 504（降级），避免占死 worker。
"""
...
from ..timeout import run_blocking_with_timeout
...
async def run_agent(req: SupervisorInput, db=Depends(get_db)) -> AgentRunResult:
    """自然语言需求 → Supervisor 判断意图 → 派发对应专家 Agent → 汇总结果。

    若结果含竞品分析，且存在活动产品，则把竞品产出回写产品空间（mod_competitor），
    供下游模块（Listing/视觉/广告）在联动时复用。
    """
    result = await run_blocking_with_timeout(SupervisorAgent().run, req.query)
...
async def run_listing(req: ListingInput, db=Depends(get_db)):
    ...
    out = await run_blocking_with_timeout(ListingAgent().run, req)
    if prod:
...
async def run_image(req: ImageGenAgentInput, db=Depends(get_db)):
    """Image Agent 直调：产品+利基 → 主图到附图的生成方案 + 构图策略。"""
    prod = load_product_for(db, req.product_id)
    if prod:
        req = fill_input(req, prod, ["product_name", "niche_keyword", "platform"])
    out = await run_blocking_with_timeout(ImageAgent().run, req)
    if prod:
...
async def run_advertising(req: AdvertisingInput, db=Depends(get_db)):
    """Advertising Agent 直调：产品+站点 → 核心指标 + 可执行广告动作 + 预算建议。"""
    prod = load_product_for(db, req.product_id)
    if prod:
        req = fill_input(req, prod, ["product_name", "niche_keyword", "country", "budget_usd"])
    out = await run_blocking_with_timeout(AdvertisingAgent().run, req)
    if prod:
...
async def run_visual(req: VisualAgentInput, db=Depends(get_db)):
    ...
    out = await run_blocking_with_timeout(VisualAgent().run, req)
    if prod:
...
async def run_voc(req: VOCInput, db=Depends(get_db)):
    ...
    out = await run_blocking_with_timeout(VOCAgent().run, req)
    if prod:
        store_module_output(
...
async def run_competitor(req: CompetitorInput, db=Depends(get_db)):
    ...
    out = await run_blocking_with_timeout(CompetitorAgent().run, req)
    if prod:
        store_module_output(
...
async def run_market_research(req: MarketResearchInput, db=Depends(get_db)):
    ...
    """
    try:
        out = await run_blocking_with_timeout(MarketResearchAgent().run, req)
